...
public abstract class EntryExitMethodLogger {
	...
	private final static List<String> EXCLUDED_DEFAULT_TYPES = Arrays
			.asList(new String[] { "javax.servlet.ServletRequest",
	...
	protected EntryExitMethodLogger(MarkerProvider markerProvider,
			DebugObjectMapper mapper, List<String> excludedCustomTypes,
			boolean excludeDefaultTypes, Object target, Method method, Object[] args) {
	 ...
	 * 1. Log method enty
	 * 2. Call method
	 * 3. Log method exit or exception if thrown
	 *
	 * @return the object
	...
	public Object logEntryExit() throws Throwable{
		...
		if (debugableArgs.length == 0) {
			...
			LOGGER.debug("Entering {}{}.{} with {}", marker, targetName,
					methodName, mapper.writeValueAsString(debugableArgs));
		}

		try {
			Object result = callRealMethod();
			if (isVoidReturned()
					|| isExcludedByDefaultType(result)
					|| isExcludedByCustomType(result)) {
				LOGGER.debug("Exiting {}{}.{}", marker, targetName, methodName);
			} else {
				LOGGER.debug("Exiting {}{}.{} with {}", marker, targetName,
						methodName, mapper.writeValueAsString(result));
			}
			return result;
		} catch (Exception e) {
			LOGGER.debug("Exiting {}{}.{} with exception {}", marker,
					targetName, methodName, e.getMessage());
			throw e;
		} catch (Error e) {
			LOGGER.debug("Exiting {}{}.{} with exception {}", marker,
					targetName, methodName, e.getMessage());
	...
	private void excludeByDefaultType(List<Object> debugableArgs) {
		...
		for (Iterator<?> iterator = debugableArgs.iterator(); iterator
				.hasNext();) {
	...
	private void excludeByAnnotationNotLogged(List<Object> debugableArgs,
			Annotation[][] paramAnnotation) {
		int argIndex = 0;
		for (Iterator<?> argIterator = debugableArgs.iterator(); argIterator
				.hasNext(); argIndex++) {
			argIterator.next();
